...
**メッシュ単位の推奨（UX改訂8で変更）**: マテリアルプロパティアニメーションはレンダラー単位でスロットを選択できないため、フェードメソッドはメッシュ（レンダラー）につき**単一**でなければならない。推奨枠は「そのレンダラーの全ての既知ファミリースロットで利用可（○または△）」な枠を優先度順で最初に採用する（共通枠なしは null = フェード不可扱い）。スロット単体の推奨は使わない（スロット別の ○△× 表示は診断用に残す）。実効枠のフォールバック（カスタム未選択時）、AO ME のグループ分割・プリセット、Toggle Menu のフェード駆動、いずれもこのメッシュ共通推奨を使う。
