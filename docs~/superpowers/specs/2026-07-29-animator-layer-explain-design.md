...
- **JSON**: 1layer = 1オブジェクト。Newtonsoft.Json（VRC SDK同梱）でシリアライズ。クリップボードコピー（相互運用）用
- **コンパクト形式**: LLM入力・キャッシュキー用の行指向テキスト（`# layer` / `param:` / `node:` / `bind:` / `edge:` 等）。JSONの括弧・引用符・キー反復を省いてトークン数を圧縮する。書式説明はsystemプロンプトに同梱
...
- **設定**: プロバイダー（OpenAI互換 / Anthropic / CLIコマンド）・エンドポイントURL・モデル名・APIキー、CLIプロバイダーはコマンドライン（`claude -p` 等。プロンプトはstdinで渡しstdoutを回収、CLI側の認証を利用するためAPIキー不要）。EditorPrefs保存（リポジトリに入らない・マシン内共有）。設定UIはAnimatorビューの歯車ボタンから
